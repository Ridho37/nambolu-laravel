    ...
    <div class="flex min-h-screen">
        <aside 
            ...
            <nav class="px-4 text-lg">
                ...
                <a href="{{ url('admin/dashboard/products') }}" class=" {{ request()->routeIs('products')||request()->routeIs('create') ? 'bg-rose-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-700' }} flex items-center px-4 py-3 mt-2 rounded-lg transition-colors duration-200">
                ...
                <a href="{{ route('messages.index') }}" class=" {{ request()->routeIs('messages.index') ? 'bg-rose-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-700' }} flex items-center px-4 py-3 mt-2 rounded-lg transition-colors duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                    <span>Pesan</span>
                </a>
                <a href="{{ route('admin.activity') }}" class="{{ request()->routeIs('admin.activity') ? 'bg-rose-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-700' }} flex items-center px-4 py-3 mt-2 rounded-lg transition-colors duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" 
                        class="h-6 w-6 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                            d="M9 17v-2a4 4 0 014-4h1a4 4 0 014 4v2M9 17H7a2 2 0 01-2-2v-1a4 4 0 014-4h4a4 4 0 014 4v1a2 2 0 01-2 2h-2"/>
                    </svg>
            ...
            <header class="bg-white shadow-sm p-4 flex justify-between items-center">
                ...
                <div class="flex items-center space-x-4">
                    <div x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false" class="relative">
                        <button @click="dropdownOpen = !dropdownOpen" class="flex items-center space-x-2 text-left">
                            <img class="h-10 w-10 rounded-full object-cover" src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=2d3748&color=fff" alt="Avatar">
                            <div class="hidden md:block">
                                <div class="font-poppins font-semibold text-sm text-slate-700">{{ auth()->user()->name }}</div>
                            </div>
                        </button>
                        <div x-show="dropdownOpen" x-transition style="display: none;" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-20 origin-top-right">
                            <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100">Profil Saya</a>
                            <form action="{{ route('logout') }}" method="post" >
                                @csrf
                                <button type="submit" class="w-full text-left block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100">Logout</button>
                            </form>
                        </div>
                    </div>
                </div>
